...
TEST_F(FaultFilterTest, ResponseRateLimit) {
  ...
  ON_CALL(encoder_filter_callbacks_, encoderBufferLimit()).WillByDefault(Return(16384));
  ...
  EXPECT_CALL(*token_timer, enableTimer(std::chrono::milliseconds(62)));
  Buffer::OwnedImpl data2(std::string(1152, 'a'));
  EXPECT_EQ(Http::FilterDataStatus::Continue, filter_->encodeData(data2, false));
  EXPECT_EQ(1024UL, data2.length());

  // Fire timer, also advance time.
  time_system_.sleep(std::chrono::milliseconds(63));
  EXPECT_CALL(*token_timer, enableTimer(std::chrono::milliseconds(62)));
  Buffer::OwnedImpl expected(std::string(64, 'a'));
  EXPECT_CALL(encoder_filter_callbacks_, encodeData(BufferEqual(&expected), false));
  ...
  time_system_.sleep(std::chrono::milliseconds(63));
  Buffer::OwnedImpl expected2(std::string(64, 'b'));
  EXPECT_CALL(encoder_filter_callbacks_, encodeData(BufferEqual(&expected2), false));
  token_timer->callback_();
  ...
  EXPECT_EQ(Http::FilterDataStatus::Continue, filter_->encodeData(data4, true));
}
